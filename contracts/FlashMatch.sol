...
contract FlashMatch is IFlashCallee {
    ...
    function flashMatch(swapRequest memory req1, swapRequest memory req2) external returns (uint256 profit) {
        ...
        // Ccalculate the largest appropriate part.
        if (req1.amount1 * availablePart1 >= req2.amount0 * availablePart2) { // Sufficient amount to borrow - scale req1
            req2.part = availablePart2; // 1 (all)
            req1.part = (10**18 * req2.amount0 / req1.amount1) * availablePart2 / availablePart1; // Round down
            ...
            require(req2.amount1 * req2.part <= req1.amount0 * req1.part, "IPDEX-FS: Unprofitable 1");
            profit = (req1.amount0 * req1.part - req2.amount1 * req2.part) / 10**18; // Does not overflow
            ...
            req2.part = (10**18 * req1.amount1 / req2.amount0) * availablePart1 / availablePart2;
            if (req1.amount1 * availablePart1 * 10**18 % req2.amount0 * availablePart2 > 0) req2.part += 1; // Round up
            // Not possible: require(req2.part <= 10**18, "IPDEX-FS: Rounding problem 2");
            IERC20(req1.token1).approve(address(ftSwap), type(uint256).max); // Refine this!
            bytes memory flashData = abi.encode(req2);
            ftSwap.swap(req1, flashData);
            require(req2.amount1 * req2.part <= req1.amount0 * req1.part, "IPDEX-FS: Unprofitable 2");
            profit = (req1.amount0 * req1.part - req2.amount1 * req2.part) / 10**18; // Does not overflow
            // Rounding dust - no need to spend gas: IERC20(req1.token1).transfer(msg.sender, IERC20(req1.token1).balanceOf(address(this)));
        }
console.log("profit");
